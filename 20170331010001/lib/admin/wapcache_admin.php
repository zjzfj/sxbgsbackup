...
class wapcache_admin extends admin {
	...
	function make_area_action() {
        ...
        if (front::post('province_id')){
        	...
        	$j=0;
        	for($i=1;$i<=$cpage;$i++){
        		$path = 'area_wap/province/'.intval(front::post('province_id')).'_list_'.$i.'.html';
        		tool::mkdir(dirname($path));
        		$data = file_get_contents(config::get('site_url').'index.php?case=area&act=list&t=wap&province_id='.intval(front::post('province_id')).'&city_id='.intval(front::post('city_id')).'&section_id='.intval(front::post('section_id')).'&page='.$i);
        		if(file_put_contents($path, $data)){
        			$j++;
        		}
        	}
        	if ($j > 0){
        		front::flash("成功生成省html <b>$j</b> 页！");
        	}else{
        		front::flash("没有生成省html！");
        	}
        }
        if (front::post('city_id')){
        	...
        	$j=0;
        	for($i=1;$i<=$cpage;$i++){
        		$path = 'area_wap/city/'.intval(front::post('city_id')).'_list_'.$i.'.html';
        		tool::mkdir(dirname($path));
        		$data = file_get_contents(config::get('site_url').'index.php?case=area&act=list&t=wap&province_id='.intval(front::post('province_id')).'&city_id='.intval(front::post('city_id')).'&section_id='.intval(front::post('section_id')).'&page='.$i);
        		if(file_put_contents($path, $data)){
        			$j++;
        	...
        	if ($j > 0){
        		front::flash("成功生成市html <b>$j</b> 页！");
        	}else{
        		front::flash("没有生成市html！");
        	}
        }
        if (front::post('section_id')){
        	...
        	$j=0;
        	for($i=1;$i<=$cpage;$i++){
        		$path = 'area_wap/section/'.intval(front::post('section_id')).'_list_'.$i.'.html';
        		tool::mkdir(dirname($path));
        		$data = file_get_contents(config::get('site_url').'index.php?case=area&act=list&t=wap&province_id='.intval(front::post('province_id')).'&city_id='.intval(front::post('city_id')).'&section_id='.intval(front::post('section_id')).'&page='.$i);
        		if(file_put_contents($path, $data)){
        			$j++;
        	...
        	if ($j > 0){
        		front::flash("成功生成区html <b>$j</b> 页！");
        	}else{
        		front::flash("没有生成区html！");
        	}
        }
