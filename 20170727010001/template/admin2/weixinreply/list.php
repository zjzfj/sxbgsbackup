  ...
  <form name="listform" id="listform"  action="<?php echo uri();?>" method="post">
    <table border="0" cellspacing="0" cellpadding="2" name="table1" id="table1" width="100%">
      ...
      <tr class="s_out" onmouseover="m_over(this)" onmouseout="m_out(this)">
		...
		<span class="hotspot" onmouseover="tooltip.show('点击编辑设置！');" onmouseout="tooltip.hide();"><a  href="<?php echo url('weixinreply/edit/id/'.$d['id']);?>" class="a_edit"></a></span>
		<span class="hotspot" onmouseover="tooltip.show('确定要删除吗？');" onmouseout="tooltip.hide();"><a onclick="javascript: return confirm('确实要删除吗?');" href="<?php echo url('weixinreply/del/wid/'.intval(front::$get['id']).'/id/'.$d['id']);?>" class="a_del"></a></span>
		</td>
      </tr>
      {/loop}
        </tbody>
          ...
          <input type="button" class="btn_a" name="button" id="button" value=" 添 加 新 词 " onclick="window.location.href='<?php echo url('weixinreply/add/wid/'.intval(front::$get['id']));?>';" />
  </form>
