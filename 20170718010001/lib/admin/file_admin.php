...
class file_admin extends admin {
    ...
    function piclist_action() {
        ...
        foreach ($img_arr as $k => $v) {
            if (in_array($k, $tmp)){
                $file = $v;
                $file = 'upload/images/'.front::get('amid').'/'.$file;
                $httpfile = config::get('site_url').$file;
                echo '<li id="albumpic'.$i.'" onclick="alselected(\'albumpic'.$i.'\',\''.$file.'\',\'selected\',1);"><p><img src="'.$httpfile.'" width="100" height="100"><span class="panel_checkbox">已选中</span></p></li>';
                $i++;
            }
        }
        echo "</ul>";
